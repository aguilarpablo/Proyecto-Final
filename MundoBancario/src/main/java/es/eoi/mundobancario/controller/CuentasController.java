...
public class CuentasController {
	...
	public ResponseEntity<String> createPrestamo(@PathVariable Integer id, @RequestBody CreatePrestamoDto dto) {
		if (prestamoService.findPrestamosVivosByNumCuenta(id).isEmpty()) {
			...
			movimientoService.
				createMovimientoPrestamo(dto, tipoMovimientoService.findIdTipoMovimiento(EnumTipoMovimiento.PRESTAMO.getValor()));
			service.updateSaldo(id, dto.getImporte(), EnumTipoMovimiento.PRESTAMO.getValor());
			return new ResponseEntity<>(HttpStatus.CREATED);
			
		} else {
			throw new Error("No se puede solicitar un préstamo si ya hay uno en curso hasta que este se haya amortizado totalmente.");
		}
	...
	public ResponseEntity<String> runAmortizaciones() {
		...
		List<Amortizacion> amortizaciones = amortizacionService.findByFecha(fecha);
		for (Amortizacion amortizacion : amortizaciones) {
			...
			movimientoService.amortizacion(amortizacion, tipoMovimientoService.
										findIdTipoMovimiento(EnumTipoMovimiento.AMORTIZACION.getValor()));
			service.updateSaldo(numCuenta, amortizacion.getImporte(), EnumTipoMovimiento.INTERES.getValor());
			movimientoService.intereses(amortizacion, tipoMovimientoService.
										findIdTipoMovimiento(EnumTipoMovimiento.INTERES.getValor()));
		}
